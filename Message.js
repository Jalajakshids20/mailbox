...
export default function Message() {
  ...
  const send = async () => {
    const messageRefSend = collection(database, "Users", `${auth.currentUser?.email}`, "Send");
    try {
      await addDoc(messageRefSend, {
        email: message
      });
    } catch (err) {
      console.error(err);
  ...
  const inbox = async () => {
    ...
    const messageRefInbox = collection(userDoc, "Inbox");
    try {
      await addDoc(messageRefInbox, {
        email: message,
        sender: auth.currentUser?.displayName
      });
      await send(); // Wait for the send function to complete
    } catch (err) {
      console.error(err);
    }
  };
  ...
  return (
    <div>
      <div onClick={handleOpen} style={{
        ...
        <img src={pen} style={{ width: "1.2vw", marginLeft: "2vw" }} alt='text' />
